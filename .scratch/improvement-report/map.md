...
- [Report scope: per-run vs rolling](issues/06-report-scope-per-run-vs-rolling.md) — per-run only; the durable artifact is `improvement-report-data.json` (committed to source control), not the HTML shell (a thin server-served view, not committed); CLI stages `improvement-report-data.json`, developer commits alongside implementation; conflict flow offers continue/stash/discard; rolling diagnostics out of scope.
...
- [Report input contract](issues/01-report-input-contract.md) — dedicated `improvement-report-data.json` written
